...
function MyActivity() {
  ...
  const [showHistory, setShowHistory] = useState(false);
  const [feedbackSession, setFeedbackSession] = useState(null);
  const [joinSession, setJoinSession] = useState(null); // State for join session modal
  const [feedbackText, setFeedbackText] = useState("");
  const [rating, setRating] = useState(0);
  const [hoverRating, setHoverRating] = useState(0);
  const { triggerNotification } = useNotification();
  ...
  const determineStatus = (sessionDate, feedbackGiven, isCanceled) => {
    ...
    if (feedbackGiven) return "Feedback Given";
    if (sessionTime < currentTime) return "Completed";
    if (sessionTime.getDate() === currentTime.getDate()) return "In Progress";
    if (sessionTime > currentTime) return "Upcoming";
    return "Upcoming";
  };

  const handleJoinSession = (session) => {
    setJoinSession(session); // Set the session to join in state
  };
    ...
    <div className="container-myactivity">
      ...
      {joinSession && (
        ...
        >
          ...
          >
            <h3 className="modal-title-myactivity">
              Join Session ({joinSession.session_date})
            </h3>
            <p className="modal-text-myactivity">
            ...
            <div className="modal-actions-myactivity">
              ...
              <button
                type="button"
                onClick={() => {
                  const meetingLink = joinSession.meeting_link; // Assuming there's a meeting link in the session data
                  window.open(meetingLink, "_blank");
                  setJoinSession(null); // Close the modal after joining
                }}
                className="join-btn-myactivity"
              >
                Join
